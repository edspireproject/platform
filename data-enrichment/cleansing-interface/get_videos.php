...
"inner join le_postmeta f on p.id = f.post_id and f.meta_key = 'wpcf-format' and f.meta_value = 'video' " .
...
$ix = 0;
foreach($f_videos as $f_video) {
	$sql .= "m" . $ix . ".meta_value as " . $f_video . ", ";
	$ix++;
}
$sql .= "'' as dummy " .
	"from le_posts p " .
...
$ix = 0;
foreach($f_videos as $f_video) {
	$sql .= "left outer join le_postmeta m" . $ix . " on p.id = m" . $ix . ".post_id and m" . $ix . ".meta_key = 'wpcf-" . str_replace('_', '-', $f_video) . "' ";
	$ix++;
}
